...
This audit crawled and parsed the *actual generated HTML* of the WaterBalanceTools repository as it exists on disk at commit `67fb7704f248c574727bcd087c880c4003c508da` -- not the source templates, and not prior phase reports. It found **525 HTML files**, of which **525** are indexable-candidate pages (the remainder are template sources and partial includes, which were excluded from the walk itself).
...
2. **Zero external sources sitewide.** Across 416 major factual pages, **416** cite zero external authoritative sources (0 `<a href>` tags anywhere in the crawled HTML point off waterbalancetools.com). The `.knowledge-sources` block present on 256 pages contains only a "Last reviewed: DATE" stamp, not a citation.
...
| Metric | Value |
|---|---|
| Total HTML files discovered (indexable-candidate walk) | 525 |
| Indexable-candidate pages | 525 |
...
Average `overall_quality_score` (0-3 scale, 10-dimension rubric, see Deliverable 2 methodology) across all 525 pages: **1.93 / 3**.

Distribution: 0-1: 0, 1-2: 296, 2-3: 229
...
- **academy/fundamentals** (9 pages): risk **LOW**, avg pairwise similarity 0, 0 high-similarity pairs, 5 repeated paragraph blocks, 0 repeated FAQ entries, 0 repeated tables.
...
4218 candidate chemistry claims (sentences containing a tracked chemistry term and/or a numeric range/unit) were extracted across 525 pages. This audit did **not** independently verify chemistry correctness against external literature -- per the audit brief, uncertain content is flagged `REQUIRES_EXPERT_REVIEW` rather than silently corrected or judged.
...
- **free chlorine**: 36 distinct range/unit strings found -- `1.5-3ppm | 1–3ppm | 3ppm | 5ppm | 67% | 1.0ppm | 0.5ppm | 0.9ppm | 2.0ppm | 1ppm | 22% | 10–30ppm | 1-3ppm | 3–5ppm | 3–6ppm | 65% | 0–0ppm | 30–50ppm | 30–100ppm | 0.5–1ppm | 400ppm | 10ppm | 78–84°f | 10% | 30ppm | 5–15ppm | 65–73% | 0ppm | 56% | 6.5ppm | 90% | 0.5–1.0ppm | 85–90°f | 60% | 80–120ppm | 75–90%`
...
- **calcium hardness**: 14 distinct range/unit strings found -- `80–120ppm | 80-100ppm | 200–400ppm | 150ppm | 80–100ppm | 400ppm | 500ppm | 10ppm | 150–250ppm | 60–70% | 3–5ppm | 300ppm | 77% | 80ppm`
- **total alkalinity**: 14 distinct range/unit strings found -- `80–120ppm | 80-100ppm | 120ppm | 1–3ppm | 80-120ppm | 31.45% | 200–400ppm | 95% | 60ppm | 6.7ppm | 93.2% | 150ppm | 80–90ppm | 80–100ppm`
- **cyanuric acid**: 12 distinct range/unit strings found -- `200ppm | 30-50ppm | 80ppm | 3ppm | 90% | 1-3ppm | 30–50ppm | 10–30ppm | 1–3ppm | 20ppm | 25% | 30ppm`
...
91 / 525 pages score LIKELY on "AI-answerable using only this page" (answer-first block + FAQ/quick-answer + sufficient semantic completeness). Full data: `aeo-audit.csv`.

## 8. E-E-A-T Findings

Missing sitewide: named_author_or_reviewer. Zero pages sitewide (0/525) carry a named author or reviewer byline. Full detail: `trust-audit.md`, `trust-audit.json`.

## 9. Structured Data Findings

Schema status counts across all JSON-LD blocks found: VALID: 952, MISSING: 39, QUESTIONABLE: 6. The MISREPRESENTED count is driven almost entirely by the `{{H1_TITLE}}` template-leakage defect (see Executive Summary). Full data: `schema-audit.csv`.

## 10. Internal Linking Findings

Orphan pages (0 inbound internal links, excluding homepage): **7**. Inbound-link distribution: {"0":7,"1-2":134,"3-4":59,"5+":325}. Full graph: `internal-link-audit.csv`, `internal-link-graph.json`.

## 11. Crawl/Indexation Findings

- CANONICAL_POINTS_ELSEWHERE: 6 page(s)
- HAS_LEGACY_REDIRECT_SOURCE_COLLISION: 6 page(s)

Sitemap contains 478 URLs against 525 discovered pages -- the -47 difference should be reconciled (either stale sitemap entries for removed pages, or a URL-form mismatch this crawl's resolver did not catch; see Audit Limitations). Full data: `crawl-indexation-audit.csv`.
...
0 / 525 pages flagged at least one static accessibility issue (missing alt text, missing viewport meta, heading-level skip, unlabeled calculator form, or a table without `<th>` headers). Full data: `ux-accessibility-audit.csv`.
...
- 416 of 416 factual pages cite zero external sources -- translating unverifiable claims multiplies the accuracy-review burden per locale.
...
Action counts: IMPROVE: 115, UNCHANGED: 300, KEEP: 84, MERGE: 26. Full matrix: `action-matrix.csv`, `action-matrix.json`.
...
Enhancement-tier only; see `action-matrix.csv` filtered to priority=P3 for the full list (433 rows).
...
7. Investigate the -47-URL gap between sitemap and crawled inventory, and the 0 sitemap/noindex contradictions.
...
- The sitemap/inventory count gap (478 vs 525) was flagged but not root-caused in this pass -- see Roadmap item 7.
- Cannibalization analysis focused on calculator/chart/guide/programmatic/reference/comparison/academy page types and a fixed list of 16 core intent phrases; it is not an exhaustive pairwise comparison of all 525 pages (that would be ~138k comparisons) and may miss cannibalization on intents outside that list.

## 24. Files and Scripts Examined

- All 525 `*.html` files under the repository root (excluding `node_modules`, `.git`, and this audit's own `reports/phase-7a/` output).
