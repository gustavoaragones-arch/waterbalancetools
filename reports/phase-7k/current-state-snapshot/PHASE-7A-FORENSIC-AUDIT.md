...
This audit crawled and parsed the *actual generated HTML* of the WaterBalanceTools repository as it exists on disk at commit `506b9ce200fc0e355d17c4a663ca02592be0d479` -- not the source templates, and not prior phase reports. It found **522 HTML files**, of which **522** are indexable-candidate pages (the remainder are template sources and partial includes, which were excluded from the walk itself).
...
Average `overall_quality_score` (0-3 scale, 10-dimension rubric, see Deliverable 2 methodology) across all 522 pages: **1.93 / 3**.

Distribution: 0-1: 0, 1-2: 294, 2-3: 228

Full scores + per-dimension evidence: `content-quality.csv`, `content-quality.json`.

## 4. Programmatic Content Findings

- **programmatic/hot-tubs** (6 pages): risk **HIGH**, avg pairwise similarity 0.705, 10 high-similarity pairs, 25 repeated paragraph blocks, 3 repeated FAQ entries, 0 repeated tables.
- **programmatic/shock** (7 pages): risk **HIGH**, avg pairwise similarity 0.697, 15 high-similarity pairs, 24 repeated paragraph blocks, 3 repeated FAQ entries, 0 repeated tables.
- **programmatic/chlorine** (12 pages): risk **HIGH**, avg pairwise similarity 0.65, 55 high-similarity pairs, 26 repeated paragraph blocks, 3 repeated FAQ entries, 0 repeated tables.
...
- **calculators** (15 pages): risk **MEDIUM**, avg pairwise similarity 0.276, 0 high-similarity pairs, 4 repeated paragraph blocks, 0 repeated FAQ entries, 0 repeated tables.
...
- **entities** (105 pages): risk **LOW**, avg pairwise similarity 0.201, 0 high-similarity pairs, 46 repeated paragraph blocks, 0 repeated FAQ entries, 0 repeated tables.
...
0 CRITICAL, 0 HIGH, 35 MEDIUM risk pairs identified among calculator/chart/guide/programmatic/reference pages sharing a core intent.
...
91 / 522 pages score LIKELY on "AI-answerable using only this page" (answer-first block + FAQ/quick-answer + sufficient semantic completeness). Full data: `aeo-audit.csv`.
...
Schema status counts across all JSON-LD blocks found: VALID: 950, MISSING: 39, QUESTIONABLE: 3. The MISREPRESENTED count is driven almost entirely by the `{{H1_TITLE}}` template-leakage defect (see Executive Summary). Full data: `schema-audit.csv`.
...
0 / 522 pages flagged at least one static accessibility issue (missing alt text, missing viewport meta, heading-level skip, unlabeled calculator form, or a table without `<th>` headers). Full data: `ux-accessibility-audit.csv`.
...
Action counts: IMPROVE: 119, UNCHANGED: 293, KEEP: 84, MERGE: 26. Full matrix: `action-matrix.csv`, `action-matrix.json`.
...
- `entities/calcium-hardness.html`: repetition_score=0: page shares several exact boilerplate paragraph/FAQ/table blocks with sibling pages despite otherwise original prose (originality_score=3) -- see programmatic-duplication.csv for family "entities".

...and 59 more; see `action-matrix.csv` filtered to priority=P2.

## 19. P3 Findings (431)

Enhancement-tier only; see `action-matrix.csv` filtered to priority=P3 for the full list (431 rows).
