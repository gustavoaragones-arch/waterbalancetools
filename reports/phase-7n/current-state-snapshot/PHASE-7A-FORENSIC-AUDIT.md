...
This audit crawled and parsed the *actual generated HTML* of the WaterBalanceTools repository as it exists on disk at commit `a6e78816aa134c59cb325c45b554fecd58325e88` -- not the source templates, and not prior phase reports. It found **524 HTML files**, of which **524** are indexable-candidate pages (the remainder are template sources and partial includes, which were excluded from the walk itself).
...
- **calculators** (15 pages): risk **MEDIUM**, avg pairwise similarity 0.273, 1 high-similarity pairs, 5 repeated paragraph blocks, 0 repeated FAQ entries, 0 repeated tables.
...
- **calcium hardness**: 14 distinct range/unit strings found -- `80–120ppm | 200–400ppm | 150ppm | 80–100ppm | 400ppm | 500ppm | 10ppm | 150–250ppm | 60–70% | 1–3ppm | 3–5ppm | 300ppm | 77% | 80ppm`
...
Orphan pages (0 inbound internal links, excluding homepage): **2**. Inbound-link distribution: {"0":2,"1-2":139,"3-4":62,"5+":321}. Full graph: `internal-link-audit.csv`, `internal-link-graph.json`.
...
Action counts: IMPROVE: 119, UNCHANGED: 293, KEEP: 84, MERGE: 28. Full matrix: `action-matrix.csv`, `action-matrix.json`.
...
- `calculators/pool-volume-calculator.html`: originality_score=1 (near-duplicate of a sibling page's body text) -- see programmatic-duplication.csv for family "calculators".
- `calculators/volume-calculator.html`: originality_score=1 (near-duplicate of a sibling page's body text) -- see programmatic-duplication.csv for family "calculators".
