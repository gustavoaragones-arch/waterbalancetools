...
(function (global, factory) {
  ...
  /**
   * WebPage schema for informational/reference pages that are not a
   * calculator, article, or entity definition (editorial policy,
   * methodology, release notes, provenance, printable resources). Minimal
   * by design -- name/description/url only, matching what these pages
   * actually are: a plain reference page, not a claim to editorial
   * authorship or article-genre content.
   */
  function generateWebPageSchema(opts) {
    var name = (opts && opts.name) || '';
    var description = (opts && opts.description) || '';
    var url = (opts && opts.url) || BASE_URL;
    var schema = {
      '@context': 'https://schema.org',
      '@type': 'WebPage',
      name: name,
  ...
  function renderAllSchemas(schemas) {
    ...
    if (schemas.webPage) {
      var wp = schemas.webPage;
      out += renderSchemaScript(wp['@type'] === 'WebPage' ? wp : generateWebPageSchema(wp));
    }
    return out;
  }
