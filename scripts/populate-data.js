 ...
 * populate-data.js
 *
 * Repeatable script that assembles structured content from the per-category
 * JS modules in scripts/data/ and writes the four compiled JSON files:
 *
 *   data/academy.json
 *   data/formulas.json
 *   data/glossary.json
 ...
 *
 * Source of truth: The files under scripts/data/ are the authoritative
 * editable sources for the four data families generated by this script:
 * academy, formulas, glossary, and reference. The corresponding files under
 * data/ are generated/compiled outputs and MUST NOT be edited directly.
 * After changing a source file under scripts/data/, run this script to
 * regenerate the corresponding JSON output.
 *
 * This script is not part of `npm run build`. Do not add it to the
 * automatic build pipeline unless a future phase establishes a safe
