/**
 * Internal linking for programmatic/shock cluster.
 */
const fs = require('fs');
const path = require('path');
const { OUTPUT_DIR, VOLUMES } = require('./shock-cluster-config');
const { insertBeforeCommonQuestions } = require('./serp-dominance-helpers');

const SLUG_RE = /^how-much-shock-for-(\d+)-gallon-pool\.html$/;
const SECTION_RE = /\s*<section class="related-shock">[\s\S]*?<\/section>\s*/i;
...
function buildSectionHtml(currentVolume, related) {
  const items = related
    .map(v => {
      const file = 'how-much-shock-for-' + v + '-gallon-pool.html';
      return (
        '        <li><a href="' +
        file +
        '">' +
    ...
    .join('\n');
  return (
    '\n    <section class="related-shock">\n' +
    '      <h2>Shock by pool size</h2>\n' +
    '      <ul class="ring-links">\n' +
    items +
    '\n      </ul>\n' +
    '    </section>\n  '
...
if (!fs.existsSync(OUTPUT_DIR)) {
  console.log('build-shock-links: skip (no OUTPUT_DIR)');
  process.exit(0);
}
...
files.forEach(file => {
  ...
  if (html.includes('class="related-shock"')) {
...
console.log('build-shock-links: updated ' + updated + ' pages in programmatic/shock/');
